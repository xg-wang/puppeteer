...
class Frame {
  ...
  async addScriptTag(options) {
    if (typeof options.url === 'string') {
      let handle;
      try {
        handle = await this._context.evaluateHandle(addScriptUrl, options.url);
      } catch (error) {
        throw new Error(`Loading script from ${options.url} failed`);
      }
      return handle;
    }
  ...
  async addStyleTag(options) {
    if (typeof options.url === 'string') {
      let handle;
      try {
        handle = await this._context.evaluateHandle(addStyleUrl, options.url);
      } catch (error) {
        throw new Error(`Loading style from ${options.url} failed`);
      }
      return handle;
    }
